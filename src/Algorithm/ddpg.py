...
class DDPG(OffPolicyAlgorithm):
    ...
    def __init__(self, *, lr, gamma, tau, update_interval, layer_units, sigma, **kwargs):
    ...
    def init_actor_loss(self):
        states_ph = self.policy.network.input
        actions_ph = self.policy.network.output
        loss = -self.critic(tf_v1.concat([states_ph, actions_ph], axis=-1))
    ...
    def action(self, states, **kwargs):
        action = super().action(states, **kwargs)
        action += self.sigma * np.random.uniform(-1, 1, size=action.shape)
        return action
    ...
    def train(self):
        ...
        self.train_critic(self.sess, states, actions, rewards, next_states, dones)
        self.train_actor(self.sess, states)
