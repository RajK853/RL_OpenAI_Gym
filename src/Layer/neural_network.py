...
class NeuralNetwork:
    ...
    def __init__(self, name, *, input_shape, layer_units, output_size, input_type=tf.float32,
                 activation=tf.nn.relu, output_activation=None, output_bias_regularizer=None,
        ...
        self.network_output = self.build_network(layer_units, activation, output_size, output_activation,
                                                 output_bias_regularizer, output_kernel_regularizer)
        self._trainable_vars = None

    def build_network(self, layer_units, activation, output_size, output_activation, output_bias_regularizer,
                      output_kernel_regularizer):
        output = self.inputs_ph
        with tf_v1.variable_scope(self.scope):
            for units in layer_units:
                output = tf.keras.layers.Dense(units, activation=activation)(output)
            output_kwargs = {"activation": output_activation,
                             "kernel_regularizer": output_kernel_regularizer,
                             "bias_regularizer": output_bias_regularizer}
            output = tf.keras.layers.Dense(output_size, **output_kwargs)(output)
        return output
...
class Q_Network(NeuralNetwork):
    ...
    def __init__(self, input_shape, output_size, scope="local_network", lr=0.001, **kwargs):
        ...
        super(Q_Network, self).__init__(scope, input_shape=input_shape, output_size=output_size, **kwargs)
